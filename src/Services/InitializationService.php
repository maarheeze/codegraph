...
final readonly class InitializationService
{
    /**
    ...
    public function run(string $projectRoot, bool $registerMcp = false, string $mcpConfig = 'auto'): array
    {
        ...
        if (!$registerMcp) {
            $lines[] = '<info>Using token-efficient CLI query commands (no MCP server registered)</info>';
    ...
    public function addClaudemdGuidelines(string $projectRoot, bool $useMcp = false): void
    {
